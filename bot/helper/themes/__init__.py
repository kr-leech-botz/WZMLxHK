...
from bot.helper.themes import tamilml_minimal

AVL_THEMES = {}
for theme in listdir('bot/helper/themes'):
    if theme.startswith('tamilml_') and theme.endswith('.py'):
...
def BotTheme(var_name, **format_vars):
    ...
    if theme_ in AVL_THEMES:
        text = getattr(AVL_THEMES[theme_].tamilmlStyle(), var_name, None)
        if text is None:
        ...
        text = getattr(rantheme.tamilmlStyle(), var_name, None)
        
    if text is None:
        text = getattr(tamilml_minimal.tamilmlStyle(), var_name)
