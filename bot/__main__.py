...
from .modules import authorize, clone, gd_count, gd_delete, gd_list, cancel_mirror, mirror_leech, status, torrent_search, torrent_select, ytdlp, \
                     ...
                     gd_clean, broadcast, category_select, autoleecher
...
async def start(client, message):
    ...
    if len(message.command) > 1 and message.command[1] == "tamilml":
...
async def log(_, message):
    buttons = ButtonMaker()
    buttons.ibutton(BotTheme('LOG_DISPLAY_BT'), f'tamilml {message.from_user.id} logdisplay')
    buttons.ibutton(BotTheme('WEB_PASTE_BT'), f'tamilml {message.from_user.id} webpaste')
...
async def bot_help(client, message):
    buttons = ButtonMaker()
    user_id = message.from_user.id
    buttons.ibutton(BotTheme('BASIC_BT'), f'tamilml {user_id} guide basic')
    buttons.ibutton(BotTheme('USER_BT'), f'tamilml {user_id} guide users')
    buttons.ibutton(BotTheme('MICS_BT'), f'tamilml {user_id} guide miscs')
    buttons.ibutton(BotTheme('O_S_BT'), f'tamilml {user_id} guide admin')
    buttons.ibutton(BotTheme('CLOSE_BT'), f'tamilml {user_id} close')
...
async def main():
    ...
    LOGGER.info(f"Tamil-ML Bot [@{bot_name}] Started!")
    if user:
        LOGGER.info(f"Tamil-ML's User [@{user.me.username}] Ready!")
